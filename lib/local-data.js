...
const REVIEWS_PATH = process.env.REVIEWS_PATH || path.join(DATA_DIR, "reviews.json");
  ...
  return records
    ...
    .map((cells, index) => {
      ...
      return {
        ...
        apiId: String(row[CSV_COLUMNS.id] || row[CSV_COLUMNS.guestId] || "").trim(),
...
function loadApplicantsFromCsv() {
  ...
  return parseApplicantsCsvText(text);
}
