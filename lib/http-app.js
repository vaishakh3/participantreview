...
const ADMIN_USERNAME = "vichu333";
const ADMIN_PASSWORD = "vichu333";
const ADMIN_COOKIE = "review_admin_session";
...
function reviewerStats(applicants, reviewerName) {
  ...
  const waitlisted = assigned.filter((item) => item.review?.decision === "waitlisted").length;
  return {
    total: assigned.length,
    approved,
    rejected,
    waitlisted,
    pending: assigned.length - approved - rejected - waitlisted
  };
}
...
function sanitizeDecision(value) {
  ...
  return ["approved", "rejected", "waitlisted"].includes(value) ? value : undefined;
}
...
function isAdminAuthenticated(request) {
  ...
  return cookies[ADMIN_COOKIE] === "authorized";
}
...
function buildLocalBootstrapState(reviewer) {
  ...
  const applicants = assignApplicantsEvenly(loadApplicantsFromCsv(), reviewers).map(({ raw, ...submission }) => ({
    ...submission,
...
async function handleApi(request, response, url) {
  ...
  if (request.method === "GET" && url.pathname === "/api/admin/session") {
    return sendJson(response, 200, { authenticated: isAdminAuthenticated(request) });
  }
  ...
  if (request.method === "POST" && url.pathname === "/api/admin/login") {
    ...
    return sendJsonWithHeaders(
      ...
      {
        "Set-Cookie": `${ADMIN_COOKIE}=authorized; Path=/; HttpOnly; SameSite=Lax; Max-Age=2592000`
      }
    );
  }
